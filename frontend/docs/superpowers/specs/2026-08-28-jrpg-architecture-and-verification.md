...
| `adventure_dungeon_states` | 解锁与各路线结算次数 | 清零 |
...
7. 若路线有效，按 unit_index 顺序生成结果；材料副本必须串行更新全局 T4 保底计数。
...
- 宝箱只出现 15/40/41/4 的 T1–T4。
- 连续 11 次无 T4 后，第 12 次必为当前目标家族 T4。
- 切换家族不重置或复制全局 miss count。
- 目标≠潮汐时长期家族分布为 50/30/5/5/5/5；目标=潮汐时为 75/5/5/5/5/5。
- 周日潮汐使用会话完成日，不使用领奖日。
- 其他三个副本永不产生 T2+。

Monte Carlo 至少使用 100 万次独立品质抽和 10 万个 20 次周目片段，实际比例应落在预先设定的统计置信区间内；第 12 次保底用穷举状态测试，不依赖概率。
...
- 所有全局上限在最极端四职业组合下仍成立。
- 四个门首领与魔王的每周目首胜各发一枚王阶催化剂，总量最多五枚。
- 胜利首奖不能重复；重复挑战不产催化剂或其他可刷取资源。
...
6. 强制连续 11 次宝箱无 T4，并在中途切换家族，确认第 12 次得到当前目标家族 T4。
